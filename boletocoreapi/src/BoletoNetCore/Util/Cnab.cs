...
namespace BoletoNetCore.Util
{
    public static class Cnab
    {
        ...
        public static Func<string, string> DominioMotivoOcorrencia_A { get; set; } =
            q =>
            {
                switch (q)
                {
                    ...
                    case "67":
                        return "Dados para Débito incompatível com a Identificação da Emissão do Boleto de Pagamento";
                    ...
                    case "97":
                        return
                            "Rejeição da alteração do prazo limite de recebimento(a data deve ser informada no campo 28.3.p)";
        ...
        ///     (Descrição C044)
        /// </summary>
        public static Func<string, string> DominioMotivoOcorrencia_B { get; } = q =>
        {
            switch (q)
            {
        ...
        /// </summary>
        public static Func<string, string> DominioMotivoOcorrencia_C { get; } = q =>
        {
            switch (q)
            {
        ...
        public static IEnumerable<string> MotivoOcorrenciaCnab240(string codigo, string codigoMovimentoRetorno)
        {
            ...
            var funcaoDominio = new[] {"02", "03", "26", "30"}.Contains(codigoMovimentoRetorno)
                ?
                DominioMotivoOcorrencia_A
                :
                new[] {"28"}.Contains(codigoMovimentoRetorno)
                    ? DominioMotivoOcorrencia_B
                    :
                    new[] {"06", "09", "17"}.Contains(codigoMovimentoRetorno)
                        ? DominioMotivoOcorrencia_C
                        : null;
            ...
            var motivos = new List<string>(5);
            for (var posicao = 0; posicao < codigo.Length - 1; posicao = posicao + 2)
            {
