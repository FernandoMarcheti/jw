...
namespace BoletoNetCore.Banco.BancoBrasil.Carteiras
{
    ...
    internal class BancoBrasilCarteira17 : ICarteira<BancoBrasil>
    {
        ...
        internal static Lazy<ICarteira<BancoBrasil>> Instance { get; } =
            new Lazy<ICarteira<BancoBrasil>>(() => new BancoBrasilCarteira17());
        ...
        public void FormataNossoNumero(Boleto.Boleto boleto)
        {
            ...
            if (IsNullOrWhiteSpace(boleto.NossoNumero))
                boleto.NossoNumero =
                    ""; //boleto registrado não aceita nosso número somente em geração do boleto, mas em remessa CNAB é possível
            //throw new Exception("Nosso Número não informado.");

            if (boleto.Banco.Beneficiario.Codigo.Length != 7)
                throw new NotImplementedException(
                    "Não foi possível formatar o nosso número: Código do Beneficiário não tem 7 dígitos.");
            ...
            if (boleto.NossoNumero?.Length == 17)
            {
                if (!boleto.NossoNumero.StartsWith(boleto.Banco.Beneficiario.Codigo))
                    throw new Exception(
                        $"Nosso Número ({boleto.NossoNumero}) deve iniciar com \"{boleto.Banco.Beneficiario.Codigo}\" e conter 17 dígitos.");
            }
            else
            {
