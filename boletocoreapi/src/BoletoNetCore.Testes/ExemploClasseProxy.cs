    ...
    public class BoletoNetCoreExemploClasseProxy
    {
        ...
        public void BoletoNetCore_ExemploProxy()
        {
            ...
            retorno = classeProxy.SetupCobranca("12.123.123/1234-46", "Beneficiario Teste Classe Proxy",
                ...
                237, "1234", "X", "", "123456", "X",
                "1213141", "0", "", "09", "",
                (int) TipoCarteira.CarteiraCobrancaSimples, (int) TipoFormaCadastramento.ComRegistro,
                (int) TipoImpressaoBoleto.Empresa, (int) TipoDocumento.Escritural,
                ref mensagemErro);
            ...
            retorno = classeProxy.DefinirPagador("123.456.789-09", "Pagador Teste Classe Proxy", "Rua Testando", "456",
                "casa 123", "Vila Central", "Cidade", "SP", "56789012", "Observação do Pagador", ref mensagemErro);
            Assert.AreEqual(true, retorno, "DefinirPagador: " + mensagemErro);

            // Define os dados do Boleto
            retorno = classeProxy.DefinirBoleto("DM", "DP123456AZ", "445566", DateTime.Now.AddDays(-3), DateTime.Now,
                DateTime.Now.AddDays(+30), (decimal) 123456.78, "CHAVEPRIMARIABANCO=12345!", "N", ref mensagemErro);
            Assert.AreEqual(true, retorno, "DefinirBoleto: " + mensagemErro);

            // Define multa (2%)
            retorno = classeProxy.DefinirMulta(classeProxy.boleto.DataVencimento,
                classeProxy.boleto.ValorTitulo * (decimal) 0.02, (decimal) 0.02, ref mensagemErro);
            Assert.AreEqual(true, retorno, "DefinirMulta: " + mensagemErro);

            // Define juros (6% ao mês)
            retorno = classeProxy.DefinirJuros(classeProxy.boleto.DataVencimento,
                classeProxy.boleto.ValorTitulo * (decimal) (0.06 / 30), (decimal) (0.06 / 30), ref mensagemErro);
            Assert.AreEqual(true, retorno, "DefinirJuros: " + mensagemErro);

            // Define desconto (5 dias antes do vencimento, R$ 10 de desconto)
            retorno = classeProxy.DefinirDesconto(classeProxy.boleto.DataVencimento.AddDays(-5), (decimal) 10.00,
                ref mensagemErro);
            ...
            retorno = classeProxy.DefinirInstrucoes("Mensagem para ser impressa no boleto",
                "Mensagem para o arquivo remessa", "01", "02", "03", "04", "05", "06", true, ref mensagemErro);
            ...
            classeProxy.GerarRemessa((int) TipoArquivo.CNAB400, nomePasta + @"ClasseProxy_Cnab400.Rem", 1,
                ref mensagemErro);
